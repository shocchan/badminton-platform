 ...
 *
 * 【画面では使っていない（2026-08-28 統合）】
 * ブログ一覧・詳細は src/pages/blogSeo.ts の pickBlogText を使う。読む列は同じだが、
 * こちらは列ごとに独立してフォールバックするため「本文は日本語なのにタイトルだけ中国語」が起き、
 * 詳細ページの canonical / hreflang / JSON-LD の判定（＝本文があるかで決まる）とズレる。
 * 表示とSEOの判定を1か所に保つため、画面からの入口は pickBlogText に寄せてある。
 */
