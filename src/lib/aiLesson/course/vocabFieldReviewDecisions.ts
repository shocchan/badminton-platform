...
export const CEO_FIELD_DECISIONS: FieldReviewDecision[] = [
  D('fi-namae:example', 'fi-namae', 'example',
    '私の名前は王小明です。／我叫王小明。', 'foundationUnit1.ts + vocabFurigana.ts', 'applied_draft'),
  D('fi-namae:meaning_zh', 'fi-namae', 'meaning_zh',
    '姓名；名字（＋usageNoteZh: 在表格或正式场合，「名前」通常指姓名。）', 'foundationUnit1.ts', 'applied_draft'),
  D('fi-komaru:meaning_zh', 'fi-komaru', 'meaning_zh',
    '为难；困扰（維持）＋usageNoteZh: 表示“不知道怎么办”的为难或困扰感。', 'foundationVocabN3.ts', 'applied_draft'),
  D('fi-kyoumi:cognate', 'fi-kyoumi', 'cognate',
    'mostly_same（維持）＋learningFocusZh追記', 'vocabularyLevelMeta.ts(維持) + vocabContentMeta.ts', 'applied_draft'),
  D('fi-genki:cognate', 'fi-genki', 'cognate',
    'partial_overlap', 'vocabularyLevelMeta.ts + vocabContentMeta.ts', 'applied_draft'),
  D('fi-kaishain:cognate', 'fi-kaishain', 'cognate',
    'japanese_specific', 'vocabularyLevelMeta.ts + vocabContentMeta.ts', 'applied_draft'),
  D('fi-kibun:cognate', 'fi-kibun', 'cognate',
    'japanese_specific（＋usageNoteZh: ≠气氛）', 'vocabularyLevelMeta.ts + foundationVocabN3.ts', 'applied_draft'),
  ...
  D('fi-yakusoku:cognate', 'fi-yakusoku', 'cognate',
    'false_friend', 'vocabularyLevelMeta.ts + vocabContentMeta.ts + N3_POOL', 'applied_draft'),
  D('fi-yasui:cognate', 'fi-yasui', 'cognate',
    'false_friend', 'vocabularyLevelMeta.ts + vocabContentMeta.ts + BASIC_POOL', 'applied_draft'),
  D('fi-zenzen:cognate', 'fi-zenzen', 'cognate',
    'partial_overlap', 'vocabularyLevelMeta.ts + vocabContentMeta.ts + N3_POOL', 'applied_draft'),
];
