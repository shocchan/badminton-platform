...
//   実測 $8.11/時間（ai_usage_daily の累計 $9.25 ÷ 1.14時間）。1か月プラン2,980円に
//   対して従来の上限（月6時間）は原価 $48＝約7,400円で、**使われるほど赤字**だった。
//
// 設計の考え方（CEO指示そのまま）:
...
export const PLAN_AI_BUDGETS: Record<PlanId, PlanAiBudget> = {
  ...
  'ai-month': {
    ...
    maxAiCostRatio: 0.45,
    rationale:
      '2,980円。音声10回（最大40分）で原価は上限でも約1,180円。'
      + '従来の「月6時間」は原価7,400円で、上限まで使われると赤字だった。'
  ...
  'coach-6m': {
    ...
    maxAiCostRatio: 0.30,
    rationale:
      '10万円。原価の主役は人間レッスン24回なので、AIは控えめに置く。'
