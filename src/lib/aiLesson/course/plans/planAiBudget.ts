...
export const PLAN_AI_BUDGETS: Record<PlanId, PlanAiBudget> = {
  'ai-trial-pass': {
    ...
    // 10→5→4（CEO決定 2026-08-28）。テキストは1日あたりの上限しかないので、
    // 7日化で原価が伸びる。
    // **5にした時点の「54.3%」は古いテキスト原価で出した数字。**
    // 2026-08-24 に「実測と呼んでいた単価は循環参照だった」と分かって模型を作り直した結果、
    // 同じ 5回/日 が 60.7%（上限60%超え）になった。4回で 58.6%。
    // 日数（7日）は削らない。翌日の復習に出会わせるのがこの商品の目的なので、
    // そこを削ると7日化した意味が消える。
    textSessionsPerDay: 4,
    maxAiCostRatio: 0.60,
    rationale:
  ...
  'ai-month': {
    planId: 'ai-month',
    // 10→8（CEO決定 2026-08-28）。10のままだと原価率48.4%で上限45%を超えていた。
    // 8で41.7%。**下げたのは音声だけ**で、テキストは8回/日のまま触っていない。
    // 音声1回（4分）は¥100、テキスト1回は¥1.8。高いのは音声だけなので、
    // 削る場所はここしかない。冒険・模試・語彙は端末内で完結して原価ゼロ。
    voiceSessionsTotal: 8,
    voiceSessionsPerDay: 2,
