...
export type PlanId = 'free-7d' | 'ai-trial-pass' | 'ai-month' | 'coach-6m';
...
export const PLAN_CATALOG: PlanConfig[] = [
  /**
   * 7日間の実力診断（無料・2026-09-10 CEO決定）。
   *
   * **料金表には出さない。** status='draft' なので publishedPlans() が返さない＝
   * LP・法務ページ・申込フォームのどこにも並ばない。招待リンクを踏んだ人にだけ現れる。
   *
   * なぜ「無料お試し」ではなく「実力診断」か:
   *   「1週間ぶん日本語が学べます」は**こちらが渡すもの**であって、相手が欲しいものではない。
   *   JLPT本番まで3か月のこの時期に在日中国語話者が欲しいのは
   *   「自分がいまどこにいるか」と「あと何をやればいいか」。それを出す道具（診断・ミニ模試・
   *   技能別準備度・錯題本）はすでに全部ある。7日は**試用期間ではなく診断が完了する期間**。
   *
   * 中身の切り方（CEO決定 2026-09-10）:
   *   - 冒険マップは最初の3地域まで（体験パスと同じ）
   *   - AI会話は**0回**。鍵付きで見せて、開けるのは回数券か有料プラン
   *   - ミニ模試は1回だけ（この上限だけ実装が別途要る。地域上限はここの値で効く）
   *
   * ここに置く理由: 権限（planEntitlements）は planCatalog から機械的に導出される。
   * カタログの外に置くと「制限なし」に落ちる（無料招待の別枠が実際そうなっている）。
   */
  {
    id: 'free-7d',
    version: 1,
    ...
    priceLabelZh: '免费',
    priceJpy: 0,
    descriptionJa: '最初の8分で、いまのJLPTの現在地が出ます。7日間、あなたが間違えた問題だけが毎日10分で戻ってきます。AI会話は含みません。',
    descriptionZh: '最初的8分钟就能测出你现在的JLPT水平。7天里，每天10分钟，只练你做错的题。不含AI会话。',
    ...
    autoRenew: false,
    featuresJa: [
      '診断（8分）で、語彙と文法の現在地が出る',
      'ミニ模試1回で、技能別の準備度が分かる',
      '間違えた問題だけが集まる錯題本',
      '7日間、教材・冒険・文法バトルが使い放題（最初の3地域）',
      '7日が終わっても、診断結果・錯題本・単語図鑑は消えません',
    ],
    featuresZh: [
      '8分钟的测试，测出你词汇和语法的现在位置',
      '1次模拟考，看清各项能力的备考程度',
      '只收集你做错的题的错题本',
      '7天内教材・冒险・语法战斗随意使用（最初的3个区域）',
      '7天结束后，测试结果・错题本・单词图鉴都不会消失',
    ],
    notIncludedJa: [
      'AI会話（回数券または有料プランで開きます）',
      '冒険マップの4地域目以降',
      '安田翔による個別レッスン',
      '個別ロードマップ',
      'WeChatでの個別学習相談',
    ...
    audienceJa: '紹介リンク・小紅書などから来た、はじめての方',
    audienceZh: '通过推荐链接・小红书等第一次来的人',
    ctaLabelJa: '無料で診断を始める',
    ctaLabelZh: '免费开始测试',
    // **draft のまま動かさないこと。** published にすると料金表に「無料」が並び、
    // ¥600 体験パスの下位互換に見える（2026-09-09 の検討でCEOが明確に避けた形）
    status: 'draft',
    ctaMode: 'apply',
    sortOrder: 5,
  },
