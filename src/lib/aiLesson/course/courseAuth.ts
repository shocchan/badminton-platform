...
import type { InviteVisit } from './inviteAnalytics';
...
export const signupWithInvite = async (
  email: string, inviteCode: string, lang: 'ja' | 'zh', wechatId: string, analytics?: InviteVisit | null,
  ...
  try {
    const res = await fetch(`${SUPA_URL}/functions/v1/ai-course-invite-signup`, {
      ...
      body: JSON.stringify({ email: email.trim(), code: inviteCode, lang, wechatId: wechatId.trim(), analytics }),
    });
