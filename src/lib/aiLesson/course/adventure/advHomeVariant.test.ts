...
describe('ホームの表示方式フラグ', () => {
  it('① 何も無ければ既定 v1', () => {
    expect(DEFAULT_HOME_VARIANT).toBe('v1');
    expect(resolveHomeVariant()).toBe('v1');
    expect(resolveHomeVariant({ search: '?home=bogus', storage: mem() })).toBe('v1');
  });

  it('② ?home=v2 で v2 になり保存される。?home=v1 で戻り、?home=reset で保存が消える', () => {
    const s = mem();
    expect(resolveHomeVariant({ search: '?home=v2', storage: s })).toBe('v2');
    expect(s.dump()).toEqual({ [HOME_VARIANT_STORAGE_KEY]: 'v2' });
    expect(resolveHomeVariant({ search: '', storage: s })).toBe('v2');
    expect(resolveHomeVariant({ search: '?home=v1', storage: s })).toBe('v1');
    expect(resolveHomeVariant({ search: '?home=reset', storage: s })).toBe('v1');
  ...
  it('③ storage が投げても落ちない', () => {
    ...
    expect(resolveHomeVariant({ search: '?home=v2', storage: broken })).toBe('v2');
    expect(resolveHomeVariant({ search: '', storage: broken })).toBe('v1');
  });
});
