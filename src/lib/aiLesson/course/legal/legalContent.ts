...
// 価格は planCatalog から組み立てた文字列で来る（商品が複数あるので数値1つでは足りない）。
// 表示は pick() で済むため、旧 yen() は廃止した。
  ...
  return [
    {
      ...
      sections: [
        ...
        {
          // 2026-08-19 追加（商品3段階化）: 買い切りプランの利用期間・利用範囲・不正利用の扱い
          heading: t('第4条 プランと利用期間', '第4条 方案与使用期限'),
          body: [
            t(
              '各プランの内容・料金・利用期間は、申込時に表示する料金表のとおりとします。買い切りプラン（AI体験パス・1か月 AI自学プラン）は自動更新されず、定められた利用期間の終了をもって、AI会話機能および対象教材の利用は終了します。',
              '各方案的内容、费用与使用期限，以报名时显示的价格表为准。一次性付费方案（AI体验通行证・1个月 AI自学方案）不会自动续费，规定的使用期限结束后，AI会话功能与对应教材的使用即告终止。',
            ),
            t(
              'AI会話機能は、受講者本人による通常の個人学習の範囲でご利用いただけます。安定した運営のため、1日および月ごとの利用回数・利用時間に上限を設けることがあります。',
              'AI会话功能仅限学员本人在正常个人学习范围内使用。为保证稳定运营，每天及每月的使用次数与时长可能设有上限。',
            ),
            t(
              'アカウントを第三者と共有・譲渡すること、および自動化されたプログラム等による大量利用を禁止します。不正利用または本規約に違反する利用が確認された場合、運営者は事前の通知なく利用を制限または停止することがあります。',
              '禁止与第三方共享或转让账号，以及通过自动化程序等方式大量使用。如确认存在滥用或违反本条款的使用行为，运营者可在不事先通知的情况下限制或停止其使用。',
            ),
          ],
        },
        {
          heading: t('第5条 禁止事項', '第5条 禁止事项'),
          body: [t(
        ...
        {
          heading: t('第8条 準拠法・管轄', '第8条 准据法与管辖'),
      ...
      sections: [
        {
          ...
          body: [t(
            'メールアドレス、ニックネーム、学習の進捗、会話の内容（音声およびテキスト）、利用状況の記録を取得します。招待からご登録の場合は、連絡・ご案内のためにWeChat IDもお預かりします。',
            '我们会收集邮箱地址、昵称、学习进度、会话内容（语音与文字）以及使用记录。通过邀请注册时，为了联系与通知，也会保存您的微信号。',
          )],
        },
        {
          heading: t('利用目的', '使用目的'),
...

    {
      ...
      sections: [
        ...
        },
        {
          ...
          body: [pick(f.priceJpyTaxIncluded, lang)],
        },
        {
