...
export const KAWABADO_LEGAL_PAGE_IDS: KawabadoLegalPageId[] = ['tokushoho', 'privacy', 'terms'];
...
/**
 * 3ページのtitle/description。**src/lib/seo/staticSeo.json と同じ文字列**を持つ。
 * Worker（scripts/generate-worker.mjs）はビルド時にJSONを読んで素のHTMLへ差し込むが、
 * 画面のコードを import できないため文言を持つ場所が2つになる。
 * ズレたら staticSeo.test.ts が落ちる（JSONの文字列がこのファイルに無いと失敗する）。
 *
 * ここを触るときは staticSeo.json も同時に直すこと。片方だけ直すと、
 * JSを実行しないクローラー（WeChat・小紅書・LINE・X）に古い文言のカードが出続ける。
 */
export const KAWABADO_LEGAL_SEO: Record<KawabadoLegalPageId, Record<Lang, { title: string; description: string }>> = {
  tokushoho: {
    ja: {
      title: '特定商取引法に基づく表記 | 川口・蕨バドミントン交流会',
      description: '川口・蕨バドミントン交流会（kawabado）の特定商取引法に基づく表記。事業者情報・参加費・お支払い方法・申込期限・キャンセルと返金の条件を掲載しています。',
    },
    zh: {
      title: '基于特定商业交易法的标示 | 川口・蕨羽毛球交流会',
      description: '川口・蕨羽毛球交流会（kawabado）基于日本特定商业交易法的标示。刊载事业者信息、参加费、支付方式、报名截止以及取消与退款的条件。',
    },
  },
  privacy: {
    ja: {
      title: 'プライバシーポリシー | 川口・蕨バドミントン交流会',
      description: '川口・蕨バドミントン交流会における個人情報の取り扱い。取得する情報・利用目的・ご案内の配信停止・委託先・保存期間・開示や削除のご請求方法をまとめています。',
    },
    zh: {
      title: '隐私政策 | 川口・蕨羽毛球交流会',
      description: '川口・蕨羽毛球交流会的个人信息处理方式。汇总了收集的信息、使用目的、停止接收通知的方法、委托方、保存期限，以及披露与删除的申请方式。',
    },
  },
  terms: {
    ja: {
      title: '利用規約 | 川口・蕨バドミントン交流会',
      description: '川口・蕨バドミントン交流会が主催する大会・通常活動の利用規約。申し込み、参加費、キャンセル、開催の変更・中止、参加者の責任と免責について定めています。',
    },
    zh: {
      title: '使用条款 | 川口・蕨羽毛球交流会',
      description: '川口・蕨羽毛球交流会主办的赛事与常规活动使用条款。规定了报名、参加费、取消、活动的变更与中止，以及参加者的责任与免责事项。',
    },
  },
};
...
export const buildKawabadoLegalPages = (
  ...
  const ja = lang === 'ja';
  const methods = (f.paymentMethods ?? []).map((m) => m[lang]).join('／');
  ...
  return [
    {
      ...
      sections: [
        ...
        {
          heading: ja ? 'お申し込みの期限・特別な条件' : '报名期限与特别条件',
          body: [pick(f.salesConditions, lang)],
        },
        {
