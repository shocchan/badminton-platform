...
export const KAWABADO_LEGAL_FACTS: KawabadoLegalFacts = {
  ...
  additionalFees: {
    ja: '銀行振込をご利用の場合の振込手数料はお客様のご負担となります。超初級ダブルス以外の大会ではシャトルのご持参が必要です（お忘れの場合は会場で1球500円でご購入いただけます）。体育館用シューズなどの用具、会場までの交通費はお客様のご負担です。',
    zh: '使用银行转账时的手续费由客户承担。除超初级双打以外的赛事需自带羽毛球（如忘记携带，可在会场以每颗500日元购买）。室内运动鞋等用具，以及前往会场的交通费由客户承担。',
  },
  paymentMethods: [
  ...
  paymentTiming: {
    ja: 'クレジットカードは申込時にお支払いいただきます。PayPay・銀行振込は、申込後にお送りするご案内メールに記載の期限までにお支払いください。追加受付を実施している大会では、オンラインのクレジットカード決済のみとなり、お申し込みと同時に決済が完了します。通常活動のお支払い方法は各活動ページに記載しています（通常活動にオンライン決済はありません）。',
    zh: '信用卡在报名时支付。PayPay・银行转账请在报名后发送的指引邮件所载期限前完成支付。设有追加报名的赛事仅限使用在线信用卡支付，报名的同时完成付款。常规活动的支付方式记载于各活动页面（常规活动不提供在线支付）。',
  },
  serviceTiming: {
  ...
  salesConditions: {
    ...
    ja: '大会のお申し込みは、原則として開催日の14日前 23:59（日本時間）で締め切ります。定員に達している場合はキャンセル待ちとしてお受けします。追加受付を実施する大会に限り、締切後もオンラインのクレジットカード決済でお申し込みいただけます。',
    zh: '赛事报名原则上于举办日的14天前23:59（日本时间）截止。名额已满时将作为候补受理。仅设有追加报名的赛事，在截止后仍可通过在线信用卡支付报名。',
  },
  refundPolicy: {
    // process-cancel/index.ts: refundAmount = entryFee - round(entryFee * 0.1)（クレカのみ自動返金）
    // CancelEntryPage.tsx: クレカ以外は「主催者より銀行振込またはPayPayにて返金」
    // CancelPolicyPage.tsx: 「返金は確認後、3〜5営業日以内に対応します」
    ja: 'キャンセル期限は原則として開催日の14日前で、各大会ページに表示しています。期限内にキャンセルされた場合、クレジットカードでお支払い済みのときはキャンセル手数料として10%を差し引いた額を同じカードへ返金します。PayPay・銀行振込でお支払い済みのときは、主催者から銀行振込またはPayPayでお支払いいただいた参加費を返金します（ご連絡の確認後3〜5営業日以内）。キャンセル期限を過ぎたキャンセル、当日キャンセル、無断欠席は返金できません。キャンセル期限後の追加受付でお申し込みいただいた分も返金できません。当会の都合で中止した場合は参加費を全額返金します。',
    zh: '取消期限原则上为举办日的14天前，并标示于各赛事页面。在期限内取消的，若已用信用卡支付，将扣除10%的取消手续费后退回原卡。若已通过PayPay・银行转账支付，主办方将通过银行转账或PayPay退还您已支付的参加费（确认联络后3〜5个工作日内）。超过取消期限的取消、当日取消及无故缺席，恕不退款。在取消期限之后通过追加报名申请的，同样不予退款。若因本会原因取消举办，将全额退还参加费。',
  },
  personalDataItems: {
