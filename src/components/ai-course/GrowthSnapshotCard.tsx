    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5">
          <Camera className="w-3.5 h-3.5 text-blue-500" />{triggerLabel(snapshot.triggerKind, tg)}
        </p>
        <p className="text-[11px] text-gray-400">{snapshot.createdAtISO.slice(0, 10)}</p>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <Metric label={tg.metricIndependent} value={<CountUp value={Math.round(m.independentRate * 100)} suffix="%" />} />
        <Metric label={tg.metricRetained} value={<CountUp value={m.retainedExpressions} />} />
        <Metric label={tg.metricRoundtrips} value={m.avgRoundtrips ? <CountUp value={m.avgRoundtrips} decimals={1} /> : '—'} />
      </div>
