...
const buildOptions = (item: FoundationItem, pool: FoundationItem[]): string[] => {
  const distractors = pool.filter(p => p.id !== item.id && p.meaningZh !== item.meaningZh).slice(0, 3)
    .map(p => p.meaningZh);
  return [item.meaningZh, ...distractors].sort((a, b) => a.localeCompare(b, 'zh'));
};
...
export const Chapter1AdventurePanel = ({ onBack }: Props) => {
  ...
  const answerLearning = (questId: string, item: FoundationItem, choice: string) => {
    ...
    setState(next);
    if (!correct) { setScreen(s => s.kind === 'learning' ? { ...s, wrongOnce: true } : s); return; }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {screen.kind === 'learning' && (() => {
        ...
        return (
          <div className="max-w-2xl bg-white rounded-2xl border border-gray-200 p-4">
            ...
            {screen.wrongOnce && <p className="text-xs text-rose-600 mb-2">もう一度。例文から意味を考えてみましょう。（再想一想，从例句找线索）</p>}
            <div className="grid grid-cols-1 gap-1.5">
              {options.map(opt => (
            ...
            <button type="button" onClick={() => setScreen({ kind: 'map' })}
              className="w-full min-h-11 mt-3 text-xs text-gray-400 underline">中断してマップへ戻る（進み具合は保存されます）</button>
          </div>
        );
      })()}
      ...
      {screen.kind === 'reviewCheck' && (() => {
        ...
        return (
          <div className="max-w-2xl bg-white rounded-2xl border border-violet-200 p-4">
            <p className="text-[11px] text-violet-500 font-bold mb-2">再会 {screen.index + 1}／{screen.keys.length}</p>
            {/* 別文脈での再確認: 初回の単語カードではなく、例文の中で意味を思い出す */}
            <p className="text-sm text-gray-800 mb-1">{item.exampleJa}</p>
            <p className="text-sm font-bold text-gray-800 mb-2">この文の「{item.displayForm}」の意味は？</p>
            {screen.wrongOnce && <p className="text-xs text-rose-600 mb-2">ゆっくりで大丈夫。文全体から思い出してみましょう。</p>}
            <div className="grid grid-cols-1 gap-1.5">
              {options.map(opt => (
                <button key={opt} type="button" onClick={() => answerReview(screen, opt === item.meaningZh)}
