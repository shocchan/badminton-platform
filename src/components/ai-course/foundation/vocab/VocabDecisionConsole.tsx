// Decision Console（Phase 2E-1.7・labPreview限定・lazy chunk）。
// 人間判断待ちを判断事項単位で確認し「ローカル判断ドラフト」を保存する画面。
// ここでの選択は human_reviewed / approved / 教材反映ではない（§0・バナーで常時明示）。
// AIのconfidence・多数決による自動確定はしない。教材データへの書き込みは一切ない。
...
import {
  createVocabDecisionRepository, DECISION_STATUSES,
} from '../../../../lib/aiLesson/course/vocabDecisionStore';
import type { DecisionDraftStatus, ImportPreview } from '../../../../lib/aiLesson/course/vocabDecisionStore';
...
export default function VocabDecisionConsole({ t, onBack }: Props) {
  ...
  return (
    <div>
      ...
      {/* 判断ドラフト＝正式承認ではない（常時表示・§0） */}
      <p className="text-[11px] leading-relaxed bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-2.5 mb-3">{td.banner}</p>
      <p className="text-xs text-gray-600 font-mono mb-2">{td.summary(summary.wordCount, summary.itemCount)}</p>
          ...
          return (
            <li key={d.decisionId} className="bg-white border border-gray-100 rounded-xl p-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${PRIO_BADGE[d.priority]}`}>{d.priority}</span>
              ...
              {open && (
                <div className="mt-3 border-t border-gray-100 pt-3 space-y-2 text-xs text-gray-700">
                  <p><span className="font-bold text-gray-500">{td.current}: </span>{d.currentValueJa}</p>
                  <p><span className="font-bold text-gray-500">{td.proposed}: </span>{d.proposedValueJa}</p>
                  <p><span className="font-bold text-gray-500">{td.source}: </span>{d.proposalSource}
                    <span className="ml-3 font-bold text-gray-500">{td.impact}: </span>{d.impactAreas.join('・')}</p>
      ...
      {/* export / import（§6・importはプレビュー→merge/replace選択。replaceは確認） */}
      <div className="mt-5 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => { setPreview(repo.previewImport(importText, new Set(queue.map((d) => d.decisionId)))); setImportMsg(''); }}
          ...
          {preview?.ok && (
            <>
              <span className="text-[11px] text-gray-600">{td.importPreview(preview.addCount ?? 0, preview.overwriteCount ?? 0)}</span>
