...
| P2-11 | quickreview空状態のCTA | ことば図鑑quickreviewが0件のとき「今は復習することばがありません。」の文言のみで次の行動ボタンがない（戻り矢印はある）。session-12実画面で確認 |
| P2-12 | 庭園→N3復習のdeep-link | 「文法のことばと再会→N3攻略へ」が該当Unit（復習語のあるUnit）ではなく現在エリアに着地する。行き止まりではないが復習語まで遠い |
...
| P2-15 | AIコース内404の戻り先 | /ja/ai-course配下の不明URLはサイト共通404（バドミントン向け導線）に落ちる。コース用の404導線は未実装 |
