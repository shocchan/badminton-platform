...
`reaction` に疑問文が入り `question` と合わせて2問。どちらに答えるか迷う。→ プロンプト規則＋**サーバー側で reaction の疑問文を機械的に落とすガード**。**2026-08-23 デプロイ済み**。本番実測で全ターン質問1つ。

### P1-3 ✅ AI会話がテーマへ誘導せず雑談で終わる
5ターン中、目標表現を使う場面が0回。→ ①「学習者の発言は受け止めるが、question は必ずテーマの場面へ引き戻す」規則 ②**直近2発話がテーマ語を含まなければ軌道修正の指示を足すサーバー側判定**。**2026-08-23 デプロイ済み**。本番実測: T2以降「ところで、さっきの場面でしたら、部長に提案するときは何と言いますか？」、T4では目標表現の型（確かに〜。ただ、〜）を提示。
