...
**管理画面の「教材」タブは 2026-08-25 に削除した**（CEO判断）。
640件を1件ずつ人が見る運用は、実際には一度も回らなかったため。

代わりに、機械チェック済みの指摘から見る。

- `docs/ai-course/adventure-v2/zh-explanation-audit.json` の有効な指摘 **101件**
  - `B_UNWANTED_JA` 4件 … 中国語の解説欄に日本語が混ざっている
  - `C_UNCLEAR` 97件 … 説明が読み取りにくい
- 640件を端から見るより、この101件を先に潰すほうが効く

判定を記録していた `ai_content_reviews` テーブルとRPCは**残してある**。
これまでの判定・メモも消していない。画面が要るときは
`AiCourseAdminPage.tsx` 冒頭のコメントに復元手順がある。
...
## 4-3. 試験対策が目的の人にはAI会話を出さない（2026-08-25 CEO決定）

**目的が jlpt・hybrid の学習者には、アプリのAI会話を一切出さない。** 会話は先生が授業でやる。
出すのは「会話が目的」（goalType='conversation'）の人だけ。

- CEOの言葉:「試験対策を選んだ人はAI会話は完全に入れないようにしよう」
  「試験対策と会話両方を目的にした人には会話が出ないようにしてみよう」
- 2026-08-22 の前の決定は「N5・N4を目標にした人には出さない」という**級**での線引きだった
  （AI会話の中身が初級の水準に届いていないため）。今回の目的での線引きがそれを覆うので、
  級の条件は消した——jlpt/hybrid は級によらず出さず、conversation は級を持たない
- 判定は1か所: `advTypes.ts` の `aiConversationAvailable(goalType)`
- そこから4か所が連動する
  1. `advRoute.ts` … hybridでも会話stageをルートに入れない（提示した道と毎日の中身を一致させる）
  2. `advQuest.ts` … 会話step・hybridの穴埋め・空クエストの逃げ道の3経路すべてを閉じる
  3. `AdvOnboarding.tsx` … 目的を選んだその場で「AI会話は出ません」と伝える／
     攻略ルート披露の「あとでAI会話で測れます」を出さない
  4. `AdvShell.tsx` … 先生画面の説明文からAI会話を外す
- 2026-08-25 より前にhybrid×N3/N2で作られた保存済みルートには会話stageが残っている。
  その人が会話stageに立っても行き止まりにならないよう、通常stageとして扱う（テストで固定）
- 検査: `advNoAiConversationExamGoal.test.ts`（目的×級×帯×時間×日付×教材の有無を総当たり）
...
- 教材をAIの判断だけで `reviewed` にする（`ai_content_reviews` は `reviewed_by` に人のメールを記録する。AIが書くと「CEOが確認した」という事実でない記録が残る）
...
| 教材の品質指摘 | `zh-explanation-audit.json` の101件（日本語混入4・説明不明瞭97）が未対応。教材タブを消したので、誰がいつ直すかの導線が無い |
