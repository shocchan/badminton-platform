...
serve(async (req) => {
  ...
  try {
    ...
    // teacherId を送ってこない旧クライアント（production frontend）には、
    // **従来と1バイトも変わらない instructions** を返す。
    // 話し方の方針は「先生を明示的に選んだリクエスト」にだけ足す（後方互換の担保）。
    const explicitTeacher = typeof body.teacherId === "string" && body.teacherId in TEACHER_VOICE;
    ...
    const params: VoicePromptParams = {
      // 既定の先生の名前は従来と同じ「翔子先生」なので、旧クライアントでも文言は変わらない
      teacherName: TEACHER_PERSONA[teacherId].name,
      teacherStyle: explicitTeacher ? TEACHER_PERSONA[teacherId].style : undefined,
