...
# ── 門(e): コードが呼ぶDBの関数・テーブルが、本番DBに本当にあるか ────────────
# 2026-09-10: 管理画面の「入金確認」を押すと404になっていた。
# ボタンは admin_set_entry_payment という関数を呼ぶが、その関数を作る migration が
# 本番DBに一度も適用されていなかった。8/28の統合から約2週間、誰も気づかなかった
# （問い合わせタブも同じ理由で動いていなかった）。
# **repo に migration ファイルがあっても、本番DBにある証拠にはならない。**
# だから出す前に、src/ が呼ぶ名前を本番DB（pg_proc / pg_class）と実際に突き合わせる。
# 確かめられなかったとき（トークンが無い・API障害）も止める。確かめずに通すと門が無いのと同じ。
# 何を「呼んでいる」とみなすかは scripts/dbObjectRefs.mjs に書いてある。
set +e
node scripts/check-db-objects.mjs src
DB_RC=$?
set -e
if [ "$DB_RC" -eq 1 ]; then
  echo "❌ (e) 本番DBに無い関数・テーブルを、コードが呼んでいます（上の一覧）"
  echo "       このまま出すと、その画面は押しても404で動きません"
  echo ""
  echo "   進めるには（どちらか）:"
  echo "     ・その関数・テーブルを作る migration を、先に本番DBへ適用する → もう一度実行"
  echo "     ・呼んでいるコードが要らないなら消してコミットする → もう一度実行"
  echo "     ※ 順番は「DBが先・コードが後」。逆にすると今回の404がそのまま起きます"
  gate_stop "本番DBに無い関数・テーブルをコードが呼んでいる"
elif [ "$DB_RC" -ne 0 ]; then
  echo "❌ (e) 本番DBと突き合わせられませんでした（上の理由）"
  echo "   進めるには: ~/.supabase_backup_token があるか・ネットにつながるかを確かめて、もう一度実行"
  gate_stop "本番DBの関数・テーブルを確認できない"
fi
echo "✅ (e) コードが呼ぶ関数・テーブルは、すべて本番DBにある"
