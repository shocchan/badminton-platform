...
<div class="hero navy">
  ...
  <div class="journey">
    ...
    <div class="leg"></div>
    <div class="stop"><div class="dot">∞</div><b>{one('毎日', '每天')}</b><span>{one('ボタン1つ 15分から', '一个按钮 15分钟起')}</span></div>
    <div class="leg"></div>
...
<section>
  ...
  <!-- 2026-08-23: 会話目標は級を1問聞いて入口を決める。目標レベルの画面は出ない -->
  <div class="note"><b>{one('「会話を伸ばしたい」をえらんだ人', '选择「提高会话能力」的人')}</b>{one(
    '目標レベルの画面は出ません。かわりに「いまの日本語はどのくらいですか？」と1問だけ聞かれます。'
    'JLPTの級を持っている人はそれを選んでください。持っている級に合った場面から会話が始まります（12問の診断もありません）。'
    'まだ受けていない人は「わからない」を選ぶと、12問で測ってから始まります。',
    '不会出现目标级别的画面。取而代之的是只问一个问题：「你的日语大概是什么水平？」。'
    '持有JLPT级别的人请选择自己的级别，会话会从与之匹配的场景开始（也不会出12道诊断题）。'
    '还没考过的人选「不清楚」，会先用12道题测量再开始。')}</div>
</section>
...
<section class="ruby-sec">
  ...
  <div class="note"><b>{one('会話について', '关于会话')}</b>{one(
    'N5・N4のあいだ、アプリのAI会話は出ません。この時期の会話は先生の授業で練習します。'
    'アプリはことば・文法・読解・聴解を積むことに集中します。',
    'N5・N4期间，应用内不会出现AI会话。这个阶段的会话在老师的课上练习。'
    '应用专注于积累词汇・语法・阅读・听力。')}</div>
</section>

<section>
  <p class="eyebrow">{eyebrow['daily']}</p>
  <h2>{one('毎日は、この青いボタンを押すだけ（1日15分から）', '每天只需按这个蓝色按钮（每天15分钟起）')}</h2>
  {t('毎日ホームを開くと「今日の一歩」が用意されています。<b>押すボタンは1つ</b>。今日やること（復習→新しい文法→問題バトル など）はAIが決めてあります。'
     '数は選んだ時間で変わります（1日5分なら1〜3つ、15分なら5〜6つ、30分なら6〜7つ）。',
     '每天打开首页，「今天的一步」已经准备好。<b>只需按一个按钮</b>。今天要做的（复习→新语法→题目战斗等）由AI安排好。'
     '数量随你选的时间变化（每天5分钟约1〜3项，15分钟约5〜6项，30分钟约6〜7项）。')}
  ...
  {t('<b>合格を本気で狙う人へ</b>: 15分は「毎日の習慣の軸」です。JLPT合格圏を目指すなら、設定で<b>1日30分</b>を選び、ミニ模試・読解/聴解も使ってください。先生の授業・宿題と合わせて、合格までの総学習量をつくります。',
     '<b>给认真冲刺合格的你</b>: 15分钟是「每天习惯的基石」。以JLPT合格为目标的话，请在设置中选择<b>每天30分钟</b>，并利用迷你模拟考、阅读/听力练习。再配合老师的课程和作业，一起积累到合格所需的总学习量。')}
...
<section class="why">
  ...
  <div class="mech">{t('<b>今日やることは、あなたの記録から選ばれています。</b>AIは「まちがえたばかりの問題」「次に必要な文法」「忘れかけていることば」を毎朝選び直します。だれにでも同じ教材を配っているのではありません。',
    '<b>今天要做的，是从你的记录里选出来的。</b>AI每天重新挑选「刚答错的题」「下一步需要的语法」「快要忘记的词」。不是给所有人发同样的教材。')}</div>
  <div class="flow">
    <div>{one('学ぶ', '学')}<span>{one('新しい文法・ことば', '新语法・生词')}</span></div>
    <div class="arrow">→</div>
    <div>{one('使う', '用')}<span>{one('バトルで確認', '战斗中确认')}</span></div>
    <div class="arrow">→</div>
    <div>{one('まちがえる', '错')}<span>{one('ノートに残る', '留在错题本')}</span></div>
    <div class="arrow">→</div>
    <div>{one('復習', '复习')}<span>{one('次の日の1つ目に出る', '第二天第一项就出')}</span></div>
  </div>
  <p class="cap">{one('まちがえた問題は「間違えた問題ノート」に残り、次の日の冒険の「復習」で出ます',
    '答错的题会留在「错题本」里，第二天冒险的「复习」就会出这些题')}</p>

  <div class="mech">{t('<b>「復習」は、あなたが間違えた問題そのものです。</b>ホームに「復習 5問」と出たら、押すとその5問がそのまま出ます。'
    '数と中身が必ず一致します。まちがえた問題が無い日は、復習は出ません。'
    '同じ問題を<b>別々の2日</b>で続けて正解できたら「克服」。同じ日に2回正解しても克服にはしません（覚えていられるかは、日をまたがないと分からないからです）。',
    '<b>「复习」就是你做错的那些题本身。</b>首页显示「复习 5题」，按下去出的就是那5题。'
    '数量和内容一定一致。没有做错的题的日子，就不会出现复习。'
    '同一道题在<b>不同的2天</b>连续答对，才算「已订正」。同一天连续答对不算（能不能记住，要隔一天才看得出来）。')}</div>
